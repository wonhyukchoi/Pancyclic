...
std::string run (std::vector<int> edges, std::string end, std::fstream &file2) {
	...
	if (listOfCycles.size() %2 == 1) {
		out+="There is an odd number of cycles, so it cannot be 2-oddly bipancyclic\n";
	} else {
		out+="The order is ";
		out+=change(listOfCycles.size()+3);
		out+= "\n";
//		std::cout << "num cycles " << listOfCycles.size() << " num arcs " << num_arcs << " num red " << num_red << std::endl;
		std::vector<std::vector<int> > bipans = biCheck(formula, num_arcs, blank, listOfCycles.size()+3, keep, doub, undoub);
		a = 0;
		if (bipans.size() > 0) {	
			out+= "This is a 2-oddly bipancyclic graph\n";
		} else {
			out+="This is not a 2-oddly bipancyclic graph\n";
		}
		while (a < bipans.size()) {
			int b = 0;
