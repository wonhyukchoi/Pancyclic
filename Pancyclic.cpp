...
std::vector<std::vector<int> > FCheck(std::vector<std::vector<int> > cycles, int num_arcs, std::vector<int> len, int order, int place, int &x, std::vector<bool> doub) {
	...
	if (place < len.size()) {
		...
		while (a < cycles.size()) {
			...
			a++;
		} if (((three == zero && three == one && (three == two || three == two+1)) || 
		(three == zero && three == one+1 && three == two+1) || 
		(three == zero+1 && three == one+1 && three == two+1))
		&& low && low2) {
			int sum = 0;
			a = 0;
			while (a < len.size()) {
...
std::string run (std::vector<int> edges, std::string end, std::fstream &file2) {
	...
	int keep = 0;
	out+="The order is ";
	out+=change(listOfCycles.size()+2);
	out+= "\n";
//	std::cout << "num cycles " << listOfCycles.size() << " num arcs " << num_arcs << " num red " << num_red << std::endl;
	std::vector<std::vector<int> > pans = TCheck(formula, num_arcs, blank, listOfCycles.size()+2, keep, doub, undoub);
	a = 0;
	if (pans.size() > 0) {	
		out+= "This is a uniquely pancyclic graph\n";
	} else {
		out+="This is not a uniquely pancyclic graph\n";
	}
	while (a < pans.size()) {
		int b = 0;
		while (b < pans[a].size()) {
