...
std::vector<std::vector<int> > biPanCheck(std::vector<std::vector<int> > cycles, int num_arcs, std::vector<int> len, int order, int place, int sum, std::vector<bool> doub, int number) {
	...
	if (place < len.size()-1) {
		...
		if (doub[place]) {
			while (a+sum <= order-1) {
				...
				temp[place]+=a;
				std::vector<std::vector<int> > out = biPanCheck(cycles, num_arcs, temp, order, place+1, sum+a, doub, number);
				int b = 0;
				while (b < out.size()) {
			...
			std::vector<std::vector<int> > out = biPanCheck(cycles, num_arcs, len, order, place+1, sum, doub, number);
			int b = 0;
			while (b < out.size()) {
				solutions.push_back(out[b]);
				b++;
...
std::vector<std::vector<int> > biECheck(std::vector<std::vector<int> > cycles, int num_arcs, std::vector<int> len, int order, int place, int &x, std::vector<bool> doub, int number) {
	std::vector<std::vector<int> > solutions;
	if (place < len.size()) {
		int a = 0;
		int b = 4;
		...
		while (a < b) {
			...
			temp[place]+= a;
			x++;
			std::vector<std::vector<int> > out = biECheck(cycles, num_arcs, temp, order, place+1, x, doub, number);
			int b = 0;
			while (b < out.size()) {
				solutions.push_back(out[b]);
				b++;
		...
		while (a < cycles.size()) {
			...
			a++;
		} if (((four == six && four == zero && (four == two || four == two+number)) || 
		(four == six && four == zero+number && four == two+number) || 
		(four == six && four == zero+number && four == two+number) ||
		(four == six+number && four == zero+number && four == two+number))
		&& low>=number && low2>=number && low3>=number) {
			int sum = 0;
			a = 0;
			while (a < len.size()) {
				sum+=len[a];
			...
			std::vector<std::vector<int> > out = biPanCheck(cycles, num_arcs, len, order, 0, sum, doub, number);
			a = 0;
			while (a < out.size()) {
...
std::vector<std::vector<int> > biFCheck(std::vector<std::vector<int> > cycles, int num_arcs, std::vector<int> len, int order, int place, int &x, std::vector<bool> doub, int number) {
	std::vector<std::vector<int> > solutions;
	if (place < len.size()) {
		int a = 0;
		int b = 2;
		...
		while (a < b) {
			std::vector<int> temp = len;
			temp[place]+= a;
			std::vector<std::vector<int> > out = biFCheck(cycles, num_arcs, temp, order, place+1, x, doub, number);
			int b = 0;
			while (b < out.size()) {
				solutions.push_back(out[b]);
				b++;
		...
		while (a < cycles.size()) {
			...
			a++;
		} if ((zero == two || zero == two+number) && four>=number) {
			int y = 0;
			x++;
			std::vector<std::vector<int> > out = biECheck(cycles, num_arcs, len, order, 0, y, doub, number);
...
std::vector<std::vector<int> > biCheck(std::vector<std::vector<int> > cycles, int num_arcs, std::vector<int> len, int order, int &x, std::vector<bool> doub, int number) {
	std::vector<std::vector<int> > solutions;
	if (len.size() < num_arcs) {
		std::vector<int> temp = len;
		temp.push_back(0);
		std::vector<std::vector<int> > out = biCheck(cycles, num_arcs, temp, order, x, doub, number);
		int a = 0;
		while (a < out.size()) {
			solutions.push_back(out[a]);
			a++;
		...
		if (doub[len.size()]) {
			temp.push_back(1);
			out = biCheck(cycles, num_arcs, temp, order, x, doub, number);
			a = 0;
			while (a < out.size()) {
				solutions.push_back(out[a]);
				a++;
		...
		if (valid) {
			...
			if (sum%2 == 0) {
				int y = 0;
				std::vector<std::vector<int> > out = biFCheck(cycles, num_arcs, len, order, 0, y, doub, number);
				x++;
...
std::string run (std::vector<int> edges, std::string end, int number) {
	...
	if (listOfCycles.size() %number != 0) {
		out+="There is an indivisible number of cycles, so it cannot be ";
		out+=change(number);
		out+="-oddly bipancyclic graph\n";
	} else {
...
//		std::cout << "num cycles " << listOfCycles.size() << " num arcs " << num_arcs << " num red " << num_red << std::endl;
		std::vector<std::vector<int> > bipans = biCheck(formula, num_arcs, blank, order, keep, doub, number);
		a = 0;
		if (bipans.size() > 0) {	
			out+= "This is an oddly ";
			out+=change(number);
			out+="-bipancyclic graph\n";
		} else {
			out+= "This is not an oddly ";
			out+=change(number);
			out+="-bipancyclic graph\n";
		}
		while (a < bipans.size()) {
