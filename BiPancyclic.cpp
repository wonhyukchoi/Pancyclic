...
std::string run (std::vector<int> edges, std::string end, std::fstream &file2) {
	...
	int keep = 0;
	out+="The order is ";
	out+=change(listOfCycles.size()*2+2);
	out+= "\n";
//	std::cout << "num cycles " << listOfCycles.size() << " num arcs " << num_arcs << " num red " << num_red << std::endl;
	std::vector<std::vector<int> > bipans = biCheck(formula, num_arcs, blank, listOfCycles.size()*2+2, keep, doub, undoub);
	a = 0;
	if (bipans.size() > 0) {	
		out+= "This is a uniquely bipancyclic graph\n";
	} else {
		out+="This is not a uniquely bipancyclic graph\n";
	}
	while (a < bipans.size()) {
		int b = 0;
