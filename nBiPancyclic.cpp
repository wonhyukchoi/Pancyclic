...
std::vector<std::vector<int> > biCheck(std::vector<std::vector<int> > cycles, int num_arcs, std::vector<int> len, int order, int &x, std::vector<bool> doub, std::vector<bool> undoub, int number) {
	std::vector<std::vector<int> > solutions;
	if (len.size() < num_arcs) {
		...
		std::vector<std::vector<int> > out = biCheck(cycles, num_arcs, temp, order, x, doub, undoub, number);
		int a = 0;
		while (a < out.size()) {
			solutions.push_back(out[a]);
			a++;
		...
		if (doub[len.size()]) {
			...
			out = biCheck(cycles, num_arcs, temp, order, x, doub, undoub, number);
			a = 0;
			while (a < out.size()) {
				solutions.push_back(out[a]);
				a++;
...
std::string run (std::vector<int> edges, std::string end, int number) {
	...
	if (listOfCycles.size()%number != 0) {
		out+="There is an indivisible number of cycles, so it cannot be a ";
		out+=change(number);
...
//	std::cout << "num cycles " << listOfCycles.size() << " num arcs " << num_arcs << " num red " << num_red << std::endl;
		std::vector<std::vector<int> > bipans = biCheck(formula, num_arcs, blank, order, keep, doub, undoub, number);
		a = 0;
		if (bipans.size() > 0) {	
			out+= "This is a ";
			out+=change(number);
