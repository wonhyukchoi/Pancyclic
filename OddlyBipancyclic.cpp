...
std::vector<std::vector<int> > biECheck(std::vector<std::vector<int> > cycles, int num_arcs, std::vector<int> len, int order, int place, int &x, std::vector<bool> doub) {
	...
	if (place < len.size()) {
		...
		while (a < cycles.size()) {
			...
			a++;
		} if (((four == six && four == zero && (four == two || four == two+1)) || 
		(four == six && four == zero+1 && four == two+1) || 
		(four == six && four == zero+1 && four == two+1) ||
		(four == six+1 && four == zero+1 && four == two+1))
		&& low && low2 && low3) {
			int sum = 0;
			a = 0;
			while (a < len.size()) {
...
std::string run (std::vector<int> edges, std::string end, std::fstream &file2) {
	...
	int keep = 0;
	out+="The order is ";
	out+=change(listOfCycles.size()*2+3);
	out+= "\n";
//	std::cout << "num cycles " << listOfCycles.size() << " num arcs " << num_arcs << " num red " << num_red << std::endl;
	std::vector<std::vector<int> > bipans = biCheck(formula, num_arcs, blank, listOfCycles.size()*2+3, keep, doub, undoub);
	a = 0;
	if (bipans.size() > 0) {	
		out+= "This is an uniquely oddly bipancyclic graph\n";
	} else {
		out+="This is not an uniquely oddly bipancyclic graph\n";
	}
	while (a < bipans.size()) {
		int b = 0;
